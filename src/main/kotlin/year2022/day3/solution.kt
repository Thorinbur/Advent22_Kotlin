...
fun Char.priority() =  (if (isLowerCase()) (this - 'a' + 1)
else this - 'A' + 27).toInt()

fun main() {
    val sacks = File("src/main/kotlin/year2022/day3/input").readLines()
    val groups = sacks.chunked(3).map { group -> group.sortedBy { it.length } }
    val prioritiesPerGroup = groups.map { group -> group.first().find { group[1].contains(it) && group[2].contains(it) }!!.priority() }
